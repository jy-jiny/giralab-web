...
def verify(base,out):
 ...
 report={'version':'1.6.2','base_url':base,'isolated_api':True,'tests':[]}
 with sync_playwright() as p:
  ...
  for width,height in [(360,640),(390,844),(412,915)]:
   ...
   page.goto(base+'?regeneration=1.6.2',wait_until='domcontentloaded')
   page.locator('.home-screen').wait_for(state='visible')
   expect(page.locator('.home-version')).to_have_text('GiraLab · 1.6.2')
   ...
   meta=ctx.request.get(base+'source-build.json?regeneration=1.6.2').json()
   assert meta['game_version']=='1.6.2' and meta['board_regeneration_mode']=='full-new'
