...
def verify(base,out):
    ...
    with sync_playwright() as p:
        ...
        for width,height in [(360,640),(390,844),(412,915)]:
            ...
            state=audio();assert state['track']=='Lunch Rush' and not state['labPlaying'],state
            ...
            assert urlsplit(media['src']).path.endswith('/audio/lunch-rush-v1.ogg') and 49.2<media['duration']<49.3,media
            ...
            music_meta=ctx.request.get(base+'audio/lunch-rush-v1.json').json()
            response=ctx.request.get(base+'audio/lunch-rush-v1.ogg')
            assert response.ok and hashlib.sha256(response.body()).hexdigest()==music_meta['ogg_sha256']
            assert music_meta['bpm']==156 and music_meta['bars']==32
            assert music_meta['peak_dbfs']<-2 and music_meta['loop_boundary_jump']<.015
            assert not errors,errors
            report['tests'].append({'viewport':[width,height],'track':'Lunch Rush','bpm':156,'duration':media['duration'],'actualLoopPlayback':True,'homeTrackUnchanged':True,'settingsOnly':True,'audioSha256':music_meta['ogg_sha256'],'errors':errors})
            ctx.close()
        browser.close()
