...
const owner={id:'00000000-0000-0000-0000-000000000501',nickname:'이메일검증'};
const errors=[],cases=[];
try{
  ...
  async function setup({fresh=false,enabled=true,loseDelete=false}={}){
    ...
    await context.route('**/*',async route=>{
      ...
      else if(endpoint==='account/status')data={enabled,linked:state.linked,player:state.player,deviceState:state.deleted?'deleted':state.player?'active':'new',devices:state.linked?2:1};
      ...
      }else if(endpoint==='account/challenge'){
        assert(enabled);assert.equal(body.email,'fixture@example.test');state.requests.set(body.requestId,{action:body.action});data={requestId:body.requestId,expiresAt:new Date(Date.now()+600000).toISOString()};
      }else if(endpoint==='account/verify'){
        assert.equal(body.code,'123456');assert.equal(body.email,'fixture@example.test');const item=state.requests.get(body.requestId);assert(item);item.verified=true;data={ready:true,action:item.action,player:owner};
  ...
  async function prove(page){
    await page.getByLabel('연결할 때 사용한 이메일').fill('fixture@example.test');await page.getByRole('button',{name:'인증번호 받기',exact:true}).click();
    await page.getByLabel('이메일로 받은 인증번호').fill('123456');await page.getByRole('button',{name:'인증번호 확인',exact:true}).click();
  ...
  await linked.page.getByRole('button',{name:'현재 기록에 이메일 연결',exact:true}).click();await expect(linked.page.locator('.home-version')).toBeVisible();
  ...
  await openAccount(linked.page);await expect(linked.page.locator('.account-panel')).toContainText('이메일 연결됨');cases.push('link preserves owner and progress');await linked.context.close();
  const recovered=await setup({fresh:true,loseDelete:true});await recovered.page.goto(base);
  await recovered.page.getByRole('button',{name:'이미 플레이했나요? 이메일로 기록 복구 · 계정 관리',exact:true}).click();await prove(recovered.page);
  await recovered.page.getByRole('button',{name:'이메일로 기록 복구',exact:true}).click();await expect(recovered.page.locator('.home-version')).toBeVisible();
  ...
  await expect(guest.page.getByRole('button',{name:'인증번호 받기'})).toBeDisabled();await guest.page.getByRole('button',{name:'계정과 기록 삭제',exact:true}).click();
  await guest.page.getByRole('button',{name:'이 기기의 기록 삭제 준비'}).click();await guest.page.getByRole('checkbox').check();
  await guest.page.getByRole('button',{name:'계정과 기록 삭제',exact:true}).click();await expect(guest.page.getByRole('heading',{name:'계정 삭제가 완료됐어요'})).toBeVisible();cases.push('guest deletion works when email readiness is off');await guest.context.close();
  const external=await setup({fresh:true,enabled:false});await external.page.goto(new URL('delete-account.html',base).href);
  await expect(external.page.getByRole('heading',{name:'GiraLab 계정 삭제'})).toBeVisible();await expect(external.page.getByRole('button',{name:'인증번호 받기'})).toBeDisabled();
